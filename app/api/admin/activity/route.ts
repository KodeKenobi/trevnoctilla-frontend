...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!token) {
      console.warn("[Admin Activity] No backend token in session, attempting to fetch from backend");
      ...
      try {
        ...
        if (tokenResponse.ok) {
          ...
          console.log("[Admin Activity] Successfully fetched token from backend");
        } else {
          console.error("[Admin Activity] Failed to fetch token from backend");
        }
      } catch (error) {
        console.error("[Admin Activity] Error fetching token:", error);
      }
      
      if (!token) {
        console.error("[Admin Activity] No backend token available");
